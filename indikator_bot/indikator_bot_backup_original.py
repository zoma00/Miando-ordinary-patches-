...
Von: Hazem Elbatawy <hazem.elbatawy@maxq.solutions>
Gesendet: Montag, 11. August 2025 19:40
Bis: amir.ibrahim@maxq.solutions <amir.ibrahim@maxq.solutions>
Betreff: indicator script
...
HISTORICAL_CONTEXT = int(os.getenv('HISTORICAL_CONTEXT', '300'))

# Build database URL
if os.path.exists(SSL_CERT_PATH):
DB_URL = f"postgresql://{DB_USER}:{DB_PASSWORD}@{DB_HOST}:{DB_PORT}/{DB_NAME}?sslmode=require&sslrootcert={SSL_CERT_PATH}"
else:
print("⚠️ SSL certificate not found, using non-SSL connection")
DB_URL = f"postgresql://{DB_USER}:{DB_PASSWORD}@{DB_HOST}:{DB_PORT}/{DB_NAME}"

print(f"🔗 Connecting to database: {DB_HOST}:{DB_PORT}/{DB_NAME}")

CONFIG = {
"adx": {"period": 14, "min_rows": 16},
"atr": {"period": 14, "min_rows": 16},
"bb": {"period": 20, "min_rows": 22},
"cci": {"period": 20, "min_rows": 22},
"ema": {"period": 20, "min_rows": 21},
"macd": {"min_rows": 35},
"obv": {"min_rows": 3},
"rsi": {"period": 14, "min_rows": 15},
"sma": {"period": 14, "min_rows": 15},
"stochastic": {"k": 14, "d": 3, "min_rows": 17},
"willr": {"period": 14, "min_rows": 15},
}

INDICATOR_COLUMNS = [
"adx", "atr", "bb_upper", "bb_middle", "bb_lower",
"cci", "ema", "macd", "macd_signal", "macd_hist",
"obv", "rsi", "sma", "stochastic_k", "stochastic_d", "willr"
]
...
print("🔁 Indikator-Bot is running...")
except Exception as e:
...
atr = tr.rolling(CONFIG["atr"]["period"]).mean()
df["atr"] = atr
...
plus_di = 100 * plus_dm.rolling(CONFIG["adx"]["period"]).mean() / atr
minus_di = 100 * minus_dm.rolling(CONFIG["adx"]["period"]).mean() / atr
dx = 100 * abs(plus_di - minus_di) / (plus_di + minus_di)
df["adx"] = dx.rolling(CONFIG["adx"]["period"]).mean()

mid = close.rolling(CONFIG["bb"]["period"]).mean()
std = close.rolling(CONFIG["bb"]["period"]).std()
df["bb_middle"] = mid
df["bb_upper"] = mid + 2 * std
df["bb_lower"] = mid - 2 * std

tp = (high + low + close) / 3
sma_tp = tp.rolling(CONFIG["cci"]["period"]).mean()
mad = tp.rolling(CONFIG["cci"]["period"]).apply(lambda x: np.mean(np.abs(x - np.mean(x))), raw=True)
df["cci"] = (tp - sma_tp) / (0.015 * mad)

df["ema"] = close.ewm(span=CONFIG["ema"]["period"], adjust=False).mean()
...
avg_gain = gain.rolling(CONFIG["rsi"]["period"]).mean()
avg_loss = loss.rolling(CONFIG["rsi"]["period"]).mean()
rs = avg_gain / avg_loss
df["rsi"] = 100 - (100 / (1 + rs))

df["sma"] = close.rolling(CONFIG["sma"]["period"]).mean()

low_min = low.rolling(CONFIG["stochastic"]["k"]).min()
high_max = high.rolling(CONFIG["stochastic"]["k"]).max()
k = 100 * (close - low_min) / (high_max - low_min)
d = k.rolling(CONFIG["stochastic"]["d"]).mean()
df["stochastic_k"] = k
df["stochastic_d"] = d

high_max = high.rolling(CONFIG["willr"]["period"]).max()
low_min = low.rolling(CONFIG["willr"]["period"]).min()
...
# Main execution loop
if __name__ == "__main__":
print("🚀 Starting Miando Indicator Bot...")

try:
while True:
combos = conn.execute(text("SELECT DISTINCT symbol, timeframe FROM ohlc_data")).fetchall()

for symbol, timeframe in combos:
df_full = pd.read_sql(text("""
SELECT * FROM ohlc_data
WHERE symbol = :symbol AND timeframe = :tf
ORDER BY open_time DESC
LIMIT :limit
"""), conn, params={"symbol": symbol, "tf": timeframe, "limit": HISTORICAL_CONTEXT})

if len(df_full) < 35:
continue

df_full = df_full.sort_values("open_time")
df_missing = df_full[df_full[INDICATOR_COLUMNS].isnull().any(axis=1)]
...
for i in reversed(range(len(df_with_indicators))):
row = df_with_indicators.iloc[i]
row_orig = df_full.iloc[i]
if row_orig[INDICATOR_COLUMNS].isnull().any():
...
if update_fields:
placeholders = ", ".join([f"{col} = :{col}" for col in update_fields])
...
sql = text(f"""
UPDATE ohlc_data SET {placeholders}
WHERE symbol = :symbol AND timeframe = :tf AND open_time = :ot
""")
result = conn.execute(sql, update_fields)

if result.rowcount == 0:
print(f"⚠️ Kein Update: {symbol} | {timeframe} | {open_time}")
else:
print(f"⬆️ Update OK: {symbol} | {timeframe} | {open_time}")

print(f"✅ Durchlauf fertig für {symbol} | {timeframe}")

print(f"⏳ Warten {SLEEP_SECONDS} Sekunden...")
...
conn.close()
print("👋 Bot-Shutdown abgeschlossen")
